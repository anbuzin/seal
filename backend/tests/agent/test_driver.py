...
async def test_single_turn_suspends_then_closes(
    ...
    run = await _start("hi")
    sid = run.run_id
    await _wait_for_lifecycle(sid, proto.SESSION_WAITING)
    ...
    await _resume(f"seal-session:{sid}:1", proto.NewUserMessage(close=True))
    output = await _wait_run(run)
    assert output.session_id == sid
    assert output.output == "hello there"
    assert not output.is_error

    assert await _lifecycle(sid) == [
    ...
    _, closed = await storage.store().info(sid, "default")
    assert closed
...
async def test_resume_appends_user_message_without_duplicating_history(
    ...
    run = await _start("one")
    sid = run.run_id
    await _wait_for_lifecycle(sid, proto.SESSION_WAITING)

    await _resume(f"seal-session:{sid}:1", proto.NewUserMessage(prompt="two"))
    await _wait_for_lifecycle(sid, proto.SESSION_WAITING, count=2)
...
async def test_gated_tool_approval_runs_in_one_turn(
    ...
    await _wait_for_lifecycle(sid, proto.TOOL_APPROVAL_REQUESTED)
...
async def test_parallel_gated_tools_park_then_run(
    ...
    await _resume_approval(
        sid, proto.ToolApprovalResponse(tool_call_id="tc-a", granted=True)
    )
    await _resume_approval(
        sid, proto.ToolApprovalResponse(tool_call_id="tc-b", granted=True)
    )
    await _wait_for_lifecycle(sid, proto.SESSION_WAITING)
    ...
    assert_message_invariants(state.messages)
    assert scripted_model.call_count == 2
    assert proto.TOOL_APPROVAL_REQUESTED in await _lifecycle(sid)
...
async def test_subagent_result_lands_on_the_trailing_tool_message(
    ...
    assert await _lifecycle(f"{sid}:child:tc-sub") == []
...
async def test_parallel_subagents_land_deterministically(
    ...
    run = await _start("delegate both")
