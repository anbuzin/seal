...
async def _capture_run(
    prompt: str, park_event: str
) -> tuple[str, list[str], list[dict[str, Any]]]:
    """Start a session, stream it like a POST /chat would, run until parked.

    Returns the session id (the run id) alongside the captures.
    """
    run = await start_session(prompt)
    ...
    return session_id, sse, ui_messages
...
async def test_create_then_chat_over_http(
    world: InProcessWorld, scripted_model: MockProvider
) -> None:
    """The client flow end to end: POST /sessions mints the id (a run id),
    then POST /chat delivers the first message as a hook resume and streams
    the turn back."""
    scripted_model.responses = [[text_msg("hello over http")]]
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        async with client.stream("POST", "/api/chat", json=body) as response:
            assert response.status_code == 200
            sse = "".join([chunk async for chunk in response.aiter_text()])

    assert "hello over http" in sse
    assert "[DONE]" in sse

    # the session id resolves to real history through the same public id.
    state = await session.read_session(session_id)
    assert state is not None
    assert state.messages[-1].text == "hello over http"
...
async def test_parallel_approvals(
    ...
    sid, sse, ui_messages = await _capture_run(
        "run both commands", proto.TOOL_APPROVAL_REQUESTED
    )

    _check_or_update("parallel-approvals", sse, ui_messages)

    # the client answers: grant tc-a, deny tc-b (fixture generated by frontend)
    approvals = await _submit_approval_request_fixture("parallel-approvals", sid)
...
async def test_parallel_subagents(
    ...
    sid, sse, ui_messages = await _capture_run(
        "delegate to two helpers", proto.SESSION_WAITING
    )
...
async def test_mixed_subagents_and_approvals(
    ...
    sid, sse, ui_messages = await _capture_run(
        "delegate and run", proto.TOOL_APPROVAL_REQUESTED
    )
    _check_or_update("mixed-subagents-approvals", sse, ui_messages)

    approvals = await _submit_approval_request_fixture("mixed-subagents-approvals", sid)
